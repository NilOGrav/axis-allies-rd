...
with open(output_file, "w", encoding="utf-8") as f:
    ...
    f.write("graph [pad=0.5, nodesep=0.5, ranksep=1.0];\n")
    f.write("""
node [
shape=box,
width=3.2,
...
""")
    ...
    for (domain, tier), nodes in domain_tier_map.items():
        ...
        f.write(f'style="rounded,dotted";\n')
        ...
        # Ranking inside the clusters
#        f.write("{ rank=same; " + " ".join(f'"{tech_id}"' for tech_id, _ in nodes) + "; }\n")
#        for i in range(len(nodes) - 1):
#            f.write(f'"{nodes[i][0]}" -> "{nodes[i+1][0]}" [style=invis, weight=0];\n')
